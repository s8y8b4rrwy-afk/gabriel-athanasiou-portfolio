...
export const SocialShare: React.FC<SocialShareProps> = ({ 
  ...
  const ShareLink = ({ 
    ...
    const baseClasses = "flex items-center gap-2 text-white hover:opacity-60 transition duration-300 cursor-pointer";
    ...
    if (href) {
      return (
        ...
        >
          {icon}
          <span className={`uppercase tracking-widest font-medium ${layout === 'vertical' ? 'text-[10px]' : 'text-xs'}`}>{label}</span>
        </a>
      );
    }
    ...
    return (
      ...
      >
        {icon}
        <span className={`uppercase tracking-widest font-medium ${layout === 'vertical' ? 'text-[10px]' : 'text-xs'}`}>
          {label}
        </span>
      </button>
    );
  ...
  if (layout === 'vertical') {
    return (
      <div className={className}>
        <p className={`${THEME.typography.meta} text-text-muted mb-3`}>Share</p>
