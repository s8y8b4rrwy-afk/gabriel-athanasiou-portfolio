...
2. **Fallback Chain**: If Cloudinary URLs fail, the system falls back to original Airtable URLs:
   ...
   - Secondary: Local WebP files (generated by `scripts/optimize-images.mjs`)
   - Tertiary: Original Airtable attachment URL
   
   > **Note on Local File Optimization**: The local WebP optimizer (`optimize-images.mjs`) uses metadata-based change detection (same as Cloudinary sync). It stores hashes in `.image-metadata.json` and only re-processes images when their Airtable metadata (id, filename, size, type) changes.
